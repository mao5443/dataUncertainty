...
public abstract class AbstractCategoryItemRenderer extends AbstractRenderer
    ...
    //modified by min
    public void drawBreakPoints(Graphics2D g2,
            CategoryPlot plot,
            Rectangle2D dataArea,
    	...
    	if (orientation == PlotOrientation.HORIZONTAL) {
    		double min = dataArea.getMinX()+ dataArea.getHeight()*(1-ratio);
    		double max = dataArea.getMaxX();
    		line = new Line2D.Double(value, min, value,
    				max);
    	}
    	else if (orientation == PlotOrientation.VERTICAL) {
    		double min = dataArea.getMinY()+ dataArea.getHeight()*(1-ratio);
...
//    	g2.setPaint(paint);
    	Stroke stroke = new BasicStroke(3f,
    	            BasicStroke.CAP_ROUND, BasicStroke.JOIN_BEVEL, 0.0f, new float[]
    	            {2.0f, 2.0f}, 0.0f);
//    	Stroke stroke = plot.getDomainGridlineStroke();
//    	
//    	if (stroke == null) {
...
//    	}
        ...
        if (orientation == PlotOrientation.HORIZONTAL) {
    		...
    		double max = dataArea.getMinX()+ dataArea.getHeight()*(1-ratio);
    		line = new Line2D.Double(value, min, value,
    				max);
    	}
    	else if (orientation == PlotOrientation.VERTICAL) {
    		double min = dataArea.getMinY();
    		double max = dataArea.getMinY()+ dataArea.getHeight()*(1-ratio);
    		line = new Line2D.Double(value, min, value,
    				max);
    	}

        stroke = new BasicStroke(3f,
    	            BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 4.0f, new float[]
    	            {2.0f, 2.0f}, 0.0f);
    	g2.setStroke(stroke);
